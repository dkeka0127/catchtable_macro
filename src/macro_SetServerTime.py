...
def check_text_on_screen():
    ...
    monitor_region = (
        monitors[1].x,
        monitors[1].y,
        monitors[1].x + monitors[1].width,
        monitors[1].y + monitors[1].height
    )
    ...
    return any(text in detected_text for text in ["예약금", "오후"])

def macro_SetServerTime():
    # 예약 날짜 좌표 추출
    root = tk.Tk()
    root.state('zoomed')
...
if __name__ == "__macro_SetServerTime__":
    macro_SetServerTime()
